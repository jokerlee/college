...
public class ChineseDocumentToSentenceProcessor implements Serializable {
  ...
  /* by jackal : full-width exclamatory mark and question mark */
  ...
  private void initNormalizationTable() {
    if (normalizationTable == null && normalizationTableFile != null) {
      ...
      for (String line : new FileLines(normalizationTableFile, encoding)) {
  ...
  */
  ...
  public static void main(String[] args) throws IOException {
    ...
    String input = StringUtils.slurpFileNoExceptions(props.getProperty("file"),
                                                     cp.encoding);
    // String input = StringUtils.slurpGBURLNoExceptions(new URL(props.getProperty("file")));

    if (props.containsKey("segmentIBM")) {
      Tokenizer<Word> tok = new WhitespaceTokenizer(new StringReader(input), true);
      ...
      String lastSgml = ""; // parse for SGML
      try {
        p1 = Pattern.compile("<.*>"); // <...>
      ...
      while (tok.hasNext()) {
        ...
        if (p2.matcher(s).matches()) {
          ...
          if (buff.toString().trim().length() > 0) {
            ...
            if (processIt) {
              /* by jackal : generate sentences list */
              List<String> sents = cp.fromPlainText(buff.toString(), true);
      ...
      List<String> sent = cp.fromHTML(input); //parseHTML
   ...
   * @throws IOException
   */
  public List<String> fromPlainText(String contentString) throws IOException {
    return fromPlainText(contentString, false);
  }

  public List<String> fromPlainText(String contentString, boolean segmented) throws IOException {
    if (segmented) {
    ...
    /* by jackal : string to char array */
    ...
    for (Character c : content) {
      ...
      /* by jackal : break document to sentences */
...
class MyHTMLParser extends HTMLEditorKit.ParserCallback {
